...
class TimeSlot {
  ...
  factory TimeSlot.fromJson(Map<String, dynamic> json) {
    ...
    return TimeSlot(
      ...
      titleContains: List<String>.from(json['title_contains'] as List? ?? []),
      isFavoritesOnly: json['is_favorites_only'] as bool? ?? false,
    );
  }
...
class ContentMetadata {
  ...
  bool matchesSlotFilters(Map<String, dynamic> filters) {
    // Content type check
    if (filters['content_type'] != null) {
      if (mediaType != filters['content_type']) return false;
    }

    // Decade check
    if (filters['decade'] != null) {
      final (decadeStart, decadeEnd) = filters['decade'] as (int, int);
      if (decade == null || decade! < decadeStart || decade! > decadeEnd) return false;
    }

    // Vote average check
    if (filters['vote_average_min'] != null) {
      if (voteAverage < filters['vote_average_min']) return false;
    }

    // Genre check
    if (filters['genres'] != null && (filters['genres'] as List).isNotEmpty) {
      final requiredGenres = Set<int>.from(filters['genres'] as List);
      if (!requiredGenres.any((g) => genres.contains(g))) return false;
    }

    // Keywords check
    if (filters['keywords'] != null && (filters['keywords'] as List).isNotEmpty) {
      final requiredKeywords = filters['keywords'] as List<String>;
      final matched = requiredKeywords.any((kw) {
        final kwLower = kw.toLowerCase();
        return title.toLowerCase().contains(kwLower) ||
            overview.toLowerCase().contains(kwLower) ||
            keywords.any((k) => k.toLowerCase().contains(kwLower));
      });
      if (!matched) return false;
    }

    // Exclude keywords check
    if (filters['exclude_keywords'] != null && (filters['exclude_keywords'] as List).isNotEmpty) {
      final excludedKeywords = filters['exclude_keywords'] as List<String>;
      for (final kw in excludedKeywords) {
        final kwLower = kw.toLowerCase();
        if (title.toLowerCase().contains(kwLower) ||
            overview.toLowerCase().contains(kwLower) ||
            keywords.any((k) => k.toLowerCase().contains(kwLower))) {
          return false;
        }
    ...
    // Universe check
    if (filters['universes'] != null && (filters['universes'] as List).isNotEmpty) {
      final requiredUniverses = filters['universes'] as List<String>;
      final matched = requiredUniverses.any((u) => universes.contains(u));
      if (!matched) return false;
    }

    // Title contains check
    if (filters['title_contains'] != null && (filters['title_contains'] as List).isNotEmpty) {
      final titlePatterns = filters['title_contains'] as List<String>;
      final matched = titlePatterns.any((pattern) {
        final patternLower = pattern.toLowerCase();
        return title.toLowerCase().contains(patternLower) ||
            originalTitle.toLowerCase().contains(patternLower) ||
            universes.any((u) => u.toLowerCase().contains(patternLower));
      });
      if (!matched) return false;
    }
